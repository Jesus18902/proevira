...
# Backend UNIFICADO: ML + Auth + Dashboard + Upload
# Migrado de arquitectura de 2 backends a 1 backend
...
# ============================================
# ENDPOINTS DE AUTENTICACIÓN (MIGRADO DE NODE.JS)
# ============================================
@app.route('/api/auth/login', methods=['POST'])
def login():
    """
    Autenticación de usuarios con bcrypt
    Migrado desde el backend Node.js eliminado
    """
    try:
        ...
        try:
            ...
            cursor.execute(
                'SELECT id_usuario, nombre, email, rol, estado, contrasena FROM usuario WHERE email = %s',
                (email,)
            )
            ...
            try:
                cursor.execute(
                    ...
                    (usuario['id_usuario'], f"Inicio de sesión - {usuario['email']}")
                )
                conn.commit()
            except Exception as bitacora_error:
                print(f"Advertencia bitácora: {bitacora_error}")
...
# ============================================
# ENDPOINTS DE CARGA DE ARCHIVOS (MIGRADO DE NODE.JS)
# ============================================
        ...
        if archivo and allowed_file(archivo.filename):
            ...
            if conn:
                try:
                    ...
                    print(f"No se pudo guardar en bitácora: {e}")
                finally:
                    cursor.close()
                    conn.close()
...
@app.route('/api/datos/procesar-csv-completo', methods=['POST'])
